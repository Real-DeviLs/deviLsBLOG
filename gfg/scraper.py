    ...
    if profilePage.status_code == 200:
        ...
        if soup.select("#School .page-content ul") != []:
            ...
            for questionTag in questionTags:
                questionList.append(questionTag.get("href"))
        ...
        try:
            ...
            response["weeklyCodingScore"] = int(mdlGrids[noOfMdlGrids - 2].text.strip()[mdlGrids[noOfMdlGrids - 2].text.index("W"):][mdlGrids[noOfMdlGrids - 2].text.strip()[mdlGrids[noOfMdlGrids - 2].text.index("W"):].index(":")+1:])
...
#     print(fetchResponse('sanskar94511'))
